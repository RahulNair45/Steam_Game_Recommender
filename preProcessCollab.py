...
for idx, row in players_and_their_games_df.iterrows(): # does this for each player
    ...
    player_games = ast.literal_eval(player_games)
    # print(player_games)
    cleaned_games = remove_bad_games(player_games)
    players_and_their_games_df.at[idx, 'library'] = cleaned_games
    num_rows += 1


players_and_their_games_df = players_and_their_games_df[pd.notna(players_and_their_games_df['library'])] # if any empty libraries after cleaning, remove those
